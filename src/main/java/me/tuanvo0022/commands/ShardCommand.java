...
public class ShardCommand extends Command {
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        ...
        if (!player.hasPermission("teacore.shard")) {
        ...
        if (args.length == 0) {
            MessageUtil.sendMessage(player, "shard_self", "%amount%", NumberUtil.formatShort(economy.getBalance(player)));
            return true;
        } 
        ...
        if (args.length == 1) {
            ...
            MessageUtil.sendMessage(player, "shard_other", "%target%", target.getName(), "%amount%", NumberUtil.formatShort(economy.getBalance(target)));
            return true;
        }
        
        MessageUtil.sendMessage(player, "shard_command");
        return true;
    }
